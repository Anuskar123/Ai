...
if ($js.Contains($oldBlankBlock)) {
    ...
    $js = [regex]::Replace($js, 'if\s*\(\s*q\.type\s*===\s*''fill-blank''\s*\)\s*\{[\s\S]*?<\/\s*div\s*>\s*`\s*;\s*\}', [regex]::Escape($newBlankBlock).Replace('\$', '$'))
    Write-Host "Updated fill-blank UI via regex."
}

# 3. Add Reset Single Question button inside Dashboard MCQ
$oldExplanationDash = @'
<div class="quiz-explanation ${isSolved ? 'show' : ''}">
'@
$newExplanationDash = @'
${isSolved ? `<div style="margin-top:12px;margin-bottom:8px;"><button class="btn btn-secondary btn-sm" onclick="if(window.resetSingleQuestion) window.resetSingleQuestion('${q.id}', 'dashboard')" style="background:#3b82f6;color:#fff;font-weight:600;padding:6px 14px;border-radius:6px;border:1px solid #60a5fa;cursor:pointer;">🔄 Reset Question & Try Again</button></div>` : ''}
                <div class="quiz-explanation ${isSolved ? 'show' : ''}">
'@
...
if ($js.Contains($oldResetFunc)) {
    ...
    $js = [regex]::Replace($js, 'function resetProgress\(\)\s*\{[\s\S]*?showToast\([^\)]*\);\s*\}\s*\}\s*catch\s*\([^\)]*\)\s*\{[\s\S]*?\}\s*\}', [regex]::Escape($newResetFunc).Replace('\$', '$'))
...
if ($js.Contains($oldCleanLatex)) {
    ...
    $js = [regex]::Replace($js, 'function cleanLatexString\(str\)\s*\{[\s\S]*?replace\(/_\\\{\[\^\}\]\+\\\}\/g,\s*''<sub>\$1<\/sub>''\);\s*\}', [regex]::Escape($newCleanLatex).Replace('\$', '$'))
    Write-Host "Upgraded cleanLatexString via regex."
}

$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($pathJs, $js, $utf8NoBom)
Write-Host "Completed app.js upgrades."
