...
if ($js -notmatch 'window\.resetSingleQuestion = function') {
    $js = $js.Replace("window.resetProgress = resetProgress;", "window.resetProgress = resetProgress;`n`n" + $resetFunc)
}

# 2. Upgrade handleBlankSubmission and renderQuizList for fill-in-the-blank so input stays enabled and has a Reset / Update button
# Let's replace fill-blank block inside renderQuizList
$oldBlankBlock = @'
...
if ($js.Contains($oldBlankBlock)) {
    ...
    Write-Host "Updated fill-blank UI inside renderQuizList."
} else {
    # Let's do a regex check if spacing differs slightly
    $js = [regex]::Replace($js, 'if\s*\(\s*q\.type\s*===\s*''fill-blank''\s*\)\s*\{[\s\S]*?<\/\s*div\s*>\s*`\s*;\s*\}', $newBlankBlock)
    Write-Host "Updated fill-blank UI via regex inside renderQuizList."
}

# 3. Add Reset Single Question button inside MCQ & Matching questions in renderQuizList (Dashboard)
$oldMcqOptionsEnd = '</div>` :' + "`n" + '                `<div class="quiz-options">'
# We will ensure every dashboard card has a reset button if isSolved is true
$oldExplanationDash = '<div class="quiz-explanation ${isSolved ? ''show'' : ''}">'
$newExplanationDash = '${isSolved ? `<div style="margin-top:12px;margin-bottom:8px;"><button class="btn btn-secondary btn-sm" onclick="if(window.resetSingleQuestion) window.resetSingleQuestion(\'${q.id}\', \'dashboard\')" style="background:#3b82f6;color:#fff;font-weight:600;padding:6px 14px;border-radius:6px;border:1px solid #60a5fa;cursor:pointer;">🔄 Reset Question & Try Again</button></div>` : \'\'}' + "`n                " + '<div class="quiz-explanation ${isSolved ? ''show'' : ''}">'
...
if ($js.Contains($oldExplanationTest1) -and -not $js.Contains('resetSingleQuestion(\'${q.id}\', \'test1\')')) {
    ...
    Write-Host "Added Reset Single button to Test 1 cards."
}

# 5. Add Reset Single Question button inside Test 2 Cards
$oldExplanationTest2 = '<div class="test1-explanation ${solved ? ''show'' : ''}" id="test2-exp-${q.id}" style="border-left:4px solid var(--primary);">'
$newExplanationTest2 = '${solved ? `<div style="margin-top:12px;margin-bottom:8px;"><button class="btn btn-secondary btn-sm" onclick="if(window.resetSingleQuestion) window.resetSingleQuestion(\'${q.id}\', \'test2\')" style="background:#3b82f6;color:#fff;font-weight:600;padding:6px 14px;border-radius:6px;border:1px solid #60a5fa;cursor:pointer;">🔄 Reset Question & Try Again</button></div>` : \'\'}' + "`n                " + '<div class="test1-explanation ${solved ? ''show'' : ''}" id="test2-exp-${q.id}" style="border-left:4px solid var(--primary);">'
...
if ($js.Contains($oldExplanationTest2) -and -not $js.Contains('resetSingleQuestion(\'${q.id}\', \'test2\')')) {
    ...
    Write-Host "Added Reset Single button to Test 2 cards."
}

# 6. Upgrade global resetProgress() so it wipes dashboard, test1, AND test2 cleanly without reloading
$oldResetFunc = @'
...
if ($js.Contains($oldResetFunc)) {
    ...
    Write-Host "Upgraded global resetProgress() to wipe all sections cleanly."
} else {
...
# 7. Upgrade cleanLatexString (`formatMathFallback`) to render pristine, textbook-grade equations with proper layout and Greek symbols
$oldCleanLatex = @'
...
Write-Host "Completed app.js reset & math engine upgrades."
